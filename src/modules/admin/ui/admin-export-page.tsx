  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[26px] bg-white p-4 shadow-sm lg:p-5">
        ...
        <p className="mt-2 text-[13px] leading-5 text-slate-600">Exporta check-ins en formato xlsx por rango de fechas, cancha y equipo.</p>
      </section>

      <section className="rounded-[26px] bg-white p-4 shadow-sm lg:p-5">
        <form className="grid gap-4 sm:max-w-2xl" onSubmit={handleSubmit}>
          <div className="grid gap-4 sm:grid-cols-2">
            <input className="rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm" type="datetime-local" value={form.from} onChange={(event) => setForm((current) => ({ ...current, from: event.target.value }))} />
            <input className="rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm" type="datetime-local" value={form.to} onChange={(event) => setForm((current) => ({ ...current, to: event.target.value }))} />
            <select className="rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm" value={form.fieldId} onChange={(event) => setForm((current) => ({ ...current, fieldId: event.target.value }))}>
